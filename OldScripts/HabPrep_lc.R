############################################################################
####    This script was developed by Jen Cruz to clean and visualize     ###    ###LC added edits to this script starting on June30,2022
...
getwd()     ####LC WD: "C:/Users/leticiacamacho/Documents/BTJR_MSProject/Rcode/Spotlights_Hab_2022/Jackrabbits"
...
datapath <- "Z:/Common/Jackrabbits/"        ##Creating pathway to call the BTJR data from the common drive
rabpath <- "BTJR_June22_Spotlights_shp/"


# Import .img file as a raster file using the "raster" package.
      ####JC code that was not working for me: 
      #shrubs <- raster::raster( paste0( habpath, 
                #"NLCD_new/TimeSeriesCover/Shrub/Shrub_2009_2020/rcmap_shrub_2020.tiff") )
#Z:\Common\QCLData\Habitat\NLCD_new\TimeSeriesCover\Shrub\Shrub_2009_2020 = location of file

shrubs <- raster::raster( paste0("Z:/Common/QCLData/Habitat/NLCD_new/TimeSeriesCover/Shrub/Shrub_2009_2020/rcmap_shrub_2020.img"))
                          

#import invasive raster
invasives <- raster::raster( paste0( "Z:/Common/QCLData/Habitat/NLCD_new/TimeSeriesCover/Annual_Herbaceous/Annual_Herbaceous_2009_2020/rcmap_annual_herbaceous_2020.img" ) )

# Check the data and that the raster loaded by making a basic plot.
#rasterVis::levelplot( shrubs )
###This plot will not work for me --- it does not give me a carrot after i run this line or any output plots 
  ###it slightly crashes session ,, even though the package is loaded 
###########FIX THIS RASTERVIS PLOT#######################


#import routes
#routes <- sf::st_read( paste0( datapath,
 #     "BTJR_KM_Markings/Km_Markings_2022/Km_Markings_2022.shp" ) )  #Projected CRS: WGS 84
routes <- sf::st_read( paste0( datapath,
...
#Projected CRS: NAD83 / UTM zone 11N + NAVD88 height
...
#####################UNDERSTAND THESE LINES BETTER HER ##########
...
#   In file(file, "rt") :
#   cannot open file 'sitedf.csv': No such file or directory
...
###################
# Start by creating a buffer around the road tracks: 
km_buf <- routes %>% 
...
st_write( alldf, "allpoints200m.shp" )
